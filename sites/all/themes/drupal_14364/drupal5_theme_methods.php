...
function drupal_14364_menu_local_task($mid, $active, $primary) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
  $link = menu_item_link($mid, FALSE);
  $output = '<span class="'.$active_class.'free-templates-lt-button-wrapper">'.
    '<span class="free-templates-lt-button-l"></span>'.
    '<span class="free-templates-lt-button-r"></span>'.
    '<a href="?q='.$link['href'].'" class="'.$active_class.'free-templates-lt-button">'.$link['title'].'</a></span>';
  return '<li>'.$output.'</li>';
}


function drupal_14364_feed_icon($url) {
  return '<a href="'. check_url($url) .'" class="free-templates-lt-rss-tag-icon" title="' . t('Syndicate content') . '"></a>';
}


function drupal_14364_comment_wrapper($content, $type = null) {
  static $node_type;
